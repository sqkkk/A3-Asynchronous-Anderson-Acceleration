...
def add_args(parser):
    ...
    parser.add_argument('--use_similarity_weight', type=int, default=1)
...
class Client(AsyncBaseClient):
    ...
    def run(self):
        ...
        if self.args.use_cv:
            ...
            self.dC = torch.zeros_like(cur_model)
...
class Server(AsyncBaseServer):
    ...
    def _weights(self):
        if not self.args.use_similarity_weight:
            return [1.0 / len(self.dW_buffer) for _ in self.dW_buffer]
        ...
        return [r / (sum(r_list) + self.epsilon) for r in r_list]
    ...
    def aggregate(self):
        ...
        if len(self.dW_buffer) >= self.buffer_size:
            ...
            if self.args.use_adam:
                ...
                model_g = self.model2tensor() + self.eta_g * dW
