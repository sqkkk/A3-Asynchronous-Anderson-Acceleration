...
class Server(AsyncBaseServer):
    def __init__(self, id, args, clients):
        ...
        # Adam-like server state for the base optimizer.
    ...
    def _buffer_update(self):
        self.aa_buffer.append({
            'client_tensor': self.cur_client.model2tensor().clone().detach(),
    ...
    def _current_period_records(self):
        ...
        # x_k is the current global iterate. Every buffered client update builds
        # a base Adam/GD iterate y_i from this same x_k.
        ...
        for entry in current_entries:
            raw_direction = entry['client_tensor'] - x_k
            stale_weight = self._staleness_weight(entry['staleness'])

            # d_base is the gradient-like descent direction that would be added
            # to x_k in a plain async method. We interpret g_i := -d_base.
            d_base = stale_weight * raw_direction
            g_i = -d_base

            # Base optimizer: Adam-like preconditioned gradient descent.
            step_t += 1
    ...
    def aggregate(self):
        ...
        # The Adam-like base optimizer has processed this whole buffered period.
