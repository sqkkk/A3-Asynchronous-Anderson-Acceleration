...
OUT_DIR="${OUT_DIR:-${ROOT_DIR}/experiments/async_distributed_compare_seed1}"
SEED="${SEED:-1}"
EPOCHS="${EPOCHS:-5}"
...
run_one() {
  local alg="$1"
  shift
  OMP_NUM_THREADS=2 MKL_NUM_THREADS=2 "${PYTHON_BIN}" "${ROOT_DIR}/async_distributed_main.py" \
    --alg "${alg}" \
    ...
    --no-cuda \
    --seed "${SEED}" \
    --dump-data "${OUT_DIR}/${alg}_seed${SEED}.pkl" \
    "$@"
}
