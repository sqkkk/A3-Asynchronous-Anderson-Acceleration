...
OUT_DIR = BASE_DIR / "cifar10_workers20_t3e200_10seed_fig"
...
METHOD_FILES = {
    "AsyncSAM+RMS": paths("cifar10_workers20_t3e200_asyncsam_rms_seed"),
    "AsyncSGD": paths("cifar10_workers20_t3e200_asyncsgd_seed"),
    "FedAsync": paths("cifar10_workers20_t3e200_fedasync_seed"),
    "FedBuff": paths("cifar10_workers20_t3e200_fedbuff_seed"),
    "FedAC": paths("cifar10_workers20_t3e200_fedac_tuned_seed"),
}
...
def load_metrics(path):
    ...
    return {
        ...
        "final_loss": float(data["test_loss"][-1]),
        "best_loss": float(min(data["test_loss"])),
    }
...
def render_with_pil(metrics_by_method, png_path, pdf_path):
    ...
    draw.text((width // 2 - 220, 20), "CIFAR-10 / ResNet18 / IID 20 Workers Tuned Protocol (10 Seeds)", fill="black", font=title_font)
    ...
    if plt is not None:
        ...
        for ax, (metric_key, std_key, title) in zip(axes, metric_specs):
            ...
            if metric_key != "final_loss":
                ax.set_ylim(top=100)
        fig.suptitle("CIFAR-10 / ResNet18 / IID 20 Workers Tuned Protocol (10 Seeds)")
        fig.tight_layout()
