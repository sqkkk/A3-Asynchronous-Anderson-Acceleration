...
SERIES = [
    ("sgd", "SGD", "#4C72B0"),
    ("adagrad", "Adagrad", "#55A868"),
    ("rmsprop", "RMSprop", "#C44E52"),
    ("adasam", "AdaSAM", "#8172B2"),
]
...
def save_pil_figure(run_data, output_dir, title):
    ...
    for tick in range(0, epochs_max + 1, 10):
        x = left_x0 + int((left_x1 - left_x0) * tick / max(epochs_max, 1))
    ...
    draw.text(((left_x0 + left_x1) // 2, y1 + 45), "Epoch", font=font, fill=(30, 30, 30), anchor="mm")

    for name, label, color in SERIES:
        rgb = _hex_to_rgb(color)
        pts = []
        for epoch, acc in zip(run_data[name]["epochs"], run_data[name]["test_prec"]):
            x = left_x0 + int((left_x1 - left_x0) * epoch / max(epochs_max, 1))
    ...
    for _, label, color in SERIES:
        rgb = _hex_to_rgb(color)
        draw.line(
            [(legend_x, legend_y + 8), (legend_x + 56, legend_y + 8)],
            fill=rgb,
            width=4,
        )
    ...
    image.save(output_dir / "mnist_compare.png")
...
def main():
    ...
    parser.add_argument("--title", type=str, default="SAM MNIST Comparison")
    ...
    with (args.output_dir / "summary.csv").open("w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["optimizer", "final_test_acc", "best_test_acc"])
        for name, label, _ in SERIES:
            writer.writerow([label, f"{run_data[name]['final_test']:.4f}", f"{run_data[name]['best_test']:.4f}"])
    ...
    try:
        ...
        fig.tight_layout()
        fig.savefig(args.output_dir / "mnist_compare.png", dpi=200, bbox_inches="tight")
        fig.savefig(args.output_dir / "mnist_compare.pdf", bbox_inches="tight")
        plt.close(fig)
    except Exception:
